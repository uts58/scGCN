...
n_clusters_for_kmaens = 2


# Function to process each chromosome
def process_chromosome(ch):
    graph_dir = f'/mmfs1/scratch/utsha.saha/mouse_data/data/datasets/wu_h_GSE239969_mus_musculus/graphs/_{ch}'
    model_dir = f"/mmfs1/scratch/utsha.saha/mouse_data/data/datasets/wu_h_GSE239969_mus_musculus/models/{ch}_no_features_100.pt"
    ...
    for n_neighbors, min_dist, n_components in product(n_neighbors_list, min_dist_list, n_components_list):
        ...
        labels_true = [i.split("_")[1].split("_")[0] for i in graph_list.keys()]
...
df.to_csv(f"wu_h_hic.csv", index=False)
